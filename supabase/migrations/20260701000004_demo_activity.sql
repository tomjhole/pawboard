-- =============================================================================
-- Redefine regenerate_demo_data so the demo Activity log reflects the real
-- state of the seeded data: payments (deposits + balances) and charges are
-- derived from the ledger / line items, alongside the existing narrative
-- entries. Everything else is unchanged from 20260701000000.
-- =============================================================================
...
begin
  ...
  -- ===========================================================================
  -- AUDIT LOG — narrative highlights + derived charges/payments so the Activity
  -- feed matches the seeded charges and payment ledger.
  -- ===========================================================================
  ...
  -- Charges — one entry per seeded line item, timed just after the booking was created.
  insert into audit_log (business_id, user_id, actor_label, action, entity_type, entity_id, meta, created_at)
  select v_biz, v_uid, 'Alex Morgan', 'charge.added', 'booking', li.booking_id,
         jsonb_build_object('description', li.description, 'total_price', li.total_price),
         b.created_at + interval '5 minutes'
  from booking_line_items li
  join bookings b on b.id = li.booking_id
  where b.business_id = v_biz;

  -- Payments — derived from the seeded ledger (deposits + settled balances).
  insert into audit_log (business_id, user_id, actor_label, action, entity_type, entity_id, meta, created_at)
  select v_biz, v_uid, 'Alex Morgan', 'payment.recorded', 'booking', p.booking_id,
         jsonb_build_object('amount', p.amount, 'method', p.method, 'kind', p.kind),
         p.paid_at
  from payments p
